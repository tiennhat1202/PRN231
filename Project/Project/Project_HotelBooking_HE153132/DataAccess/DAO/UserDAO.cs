...
namespace DataAccess.DAO
{
    public class UserDAO : IUserDAO
    {
        ...
        public User GetUserLogin(string email, string password)
        {
            ...
            try
            {
                if (email.Equals("")) throw new Exception("Email can not be empty");
                if (password.Equals("")) throw new Exception("Password can not be empty");
                var findUser = _dbContext.Users.Where(u => u.Email.Equals(email) && u.Password.Equals(password)).Count();
                if (findUser == 0)
                {
                    user = null;
                }
                ...
                {
                    user = _dbContext.Users.Where(u => u.Email.Equals(email) && u.Password.Equals(password)).First();
                }
            }
            catch (Exception e)
            {
        ...
        public User GetEmailResetPassword(string email)
        {
            User? user = new User();
            try
            {
                user = _dbContext.Users.SingleOrDefault(x => x.Email.Equals(email));
            }
            catch (Exception ex)
            {
                throw new Exception(ex.Message);
